...
struct NaturalLanguageParser {
    ...
    func parse(_ input: String) -> ParsedReminderInput {
        ...
        stripped = stripped
            ...
            .replacingOccurrences(of: "daily", with: "", options: .caseInsensitive)
            .replacingOccurrences(of: "weekly", with: "", options: .caseInsensitive)
            .replacingOccurrences(of: "monthly", with: "", options: .caseInsensitive)
            .replacingOccurrences(of: "every day", with: "", options: .caseInsensitive)
            .replacingOccurrences(of: "every week", with: "", options: .caseInsensitive)
            .replacingOccurrences(of: "every month", with: "", options: .caseInsensitive)
            .replacingOccurrences(of: "section", with: "", options: .caseInsensitive)
            .replacingOccurrences(of: "focus mode", with: "", options: .caseInsensitive)
        ...
        stripped = prettyTitle(stripped)

        return ParsedReminderInput(
            cleanedTitle: stripped.isEmpty ? prettyTitle(input) : stripped,
    ...
    private func prettyTitle(_ text: String) -> String {
        let smallWords = Set(["and", "or", "to", "for", "of", "the", "a", "an", "in", "on", "at"])
        let parts = text
            .split(separator: " ")
            .map(String.init)
            .enumerated()
            .map { idx, word -> String in
