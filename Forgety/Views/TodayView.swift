...
struct TodayView: View {
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 12) {
                        ...
                        HStack(spacing: 10) {
                            visualMetric("Overdue", value: store.overdueReminders.count, icon: "exclamationmark.triangle.fill", tint: .red)
                            visualMetric("Today", value: store.todaysReminders.count, icon: "sun.max.fill", tint: .orange)
                            visualMetric("Done", value: store.completionToday, icon: "checkmark.seal.fill", tint: .green)
                        }
    ...
    private func visualMetric(_ title: String, value: Int, icon: String, tint: Color) -> some View {
        GlassCard {
            ...
            HStack {
                ...
                VStack(alignment: .leading) {
                    Text("Due This Week")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    Text("\(store.weekReminders.count)")
                        .font(.title3.weight(.bold))
                }
            }
        }
